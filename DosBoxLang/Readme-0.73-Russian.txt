...
Обновление перевода для версии 0.73: Alexander Katz (skatz@svitonline.com)
...
В: У моей игры на движке Build (Duke3D/Blood/Shadow Warrior) есть проблемы.
...
О: Чтобы монтировать CD-ROM в DOSBox вам надо указать некоторые дополнительные
   ...
   Чтобы включить поддержку CD-ROM (включает MSCDEX)::
   ...
   например, если не работает CD-аудио.
     ...
     Чтобы включить доступ ioctl, используя цифровое извлечений аудио для CD-аудио
     (только Windows, полезно под Vista):
     ...
     Чтобы принудительно включить доступ толко через ioctl (только Windows):
       - mount d f:\ -t cdrom -ioctl_dio
     Чтобы включить низкоуровневую поодержку aspi (Win98 с установленным aspi-layer):
...
О: Убедитесь, что смонтировали CD-ROM с -t cdrom параметром, это задействует 
   ...
   описанию команды mount в разделе 4 за разъяснением их значения и описанием
   дополнительных возмодностей для аудио-CD: -ioctl_dx, ioctl_mci, ioctl_dio.
    
   Попробуйте создать образ CD-ROM (желательно пару CUE/BIN) и используйте 
   внутреннюю  утилиту DOSBox IMGMOUNT, чтобы смонтировать образ (файл СГУ).
...
О: Убедитесь, что звук корректно настроен в игре. Это может быть сделано во время 
   ...
   В некоторых случаях может быть полезно испоьзовать другое эмулируемое 
   устройство, например SoundBlastr Pro (sbtype=sbpro1 в настроечном файле 
...
A: Это может призойти в различных ситуациях, например, если клавиатурная раскладка 
   вашего компьютера не имеет соответствующей DOS раскладки (или была неверно
   определена), или файл переназаначения клавиш неверен.
   Возможные решения проблемы:
     1. Используйте  / вместо \, или ALT-58 для : и ALT-92 для \.
     2. Измените раскладку клавиатуры DOS'а (см. раздел 7 Клавиатурная раскладка).
   ...
   стандартная раскладка US. В этом случае попобуйте клавиши рядом с "enter" 
   для клавиши \ (обратная косая), а для клавиши : (двоеточие) используйте 
   shift и клавиши между"enter" и"l".
      
В: Клавиатура тормозит.
О: Уменьшите настройки приоритета в конфигурационном файле DOSBox, например,
   выставив "priority=normal,normal". Вы можете также попробовать уменьшить 
   число циклов. (Начните с фиксированного числа циклов, скажем cycles=10000).
...
В: Игра/приложение вообще не работает/вылетает!
О: См. раздел 10: Решение проблем
...
О: Это возможно! Просто создайте конфигурационный файл: config -writeconf 
   ...
   файл_настроек.
   Обратитесь к описанию команды config в разделе 4 за дополнительными сведениями.
...
О: DOSBox эмулирует несколько существующих звуковых устройств:
   ...
   - Adlib
     Этот эмулятор почти безупречный и включает способность Adlib 
     проигрывать оцифрованный звук.
...
В: У моей игры на движке Build (Duke3D/Blood/Shadow Warrior) есть проблемы.
A: Прежде всего, попытайтеь найти версию игры портированную под вашу систему.
   Это даст вам большие возможности. Чтобы решить проблемы с графикой высокого 
   разрешения в DOSBox, откройте настроечный файл DOSBox и пощите в нем строчку
   machine=svga_s3. Замените svga_s3 на vesa_nolfb
...
диск C: и этот файл будет запущен.
  ...
  -conf configfile
        ...
        Смотрите часть 11 для подробностей.
  ...
  -machine machinetype
        ...
        hercules, cga, pcjr, tandy, svga_s3 (по-умолчанию), а также дополнительные
        svga карты, перечисленные в комментариях к настроечному файлу DOSBox.
  ...
  -securemode
        ...
        AUTOEXEC.BAT (который, в сою очередь, запрещает любые изменения
  ...
  -editconf program
        Вызывает программу, передав ей настроечный файл в качестве первого параметра.
        Вы можете задать эту команду несколько раз. В этом случае вторая
        программа будет вызвана, если первая не запустится.
...
MOUNT -u "Буква эмулируемого диска"
  ...
  -label drivelabel
        ...
        если имя диска не читается корректно (полезно, когда программа не может найти свой  
        CD-ROM). Если не указано имя и не включена поддержка на низком уровне 
        (не указаны -usecd # и/или -aspi параметры или указано -noioctl):
  ...
  -ioctl_mci (используется MCI для CD-аудио)
        ...
        Варианты различаются тлько способом работы с CD-аудио,
  ...
  -usecd number
        ...
        Позволяеет указать диск, который будет использоваться SDL. Используйте,
        если при использвании SDL интерфейса монтируется неверный CD-ROM диск 
...
LOADFIX -f
  Программа для сокращения количества доступной обычной памяти. Полезна для старых 
...
MIXER
  ...
  /LISTMIDI
      ...
      добавьте строку
      'midiconfig=id' к секции [midi] в настроечном файле, где
...
IMGMOUNT
  ...
  imagefile
      ...
      также монтировать образы дисков CD-ROM (ISO или CUE/BIN), если хотите 
      иметь возможность переключать CD, укажите все образы в виде 
      последовательности (см. следующий пункт).
      Предпочтельным типом образов дтсков CD-ROM является пара CUE/BIN,
      так как она может содержать аудио дорожки в отличие от образов
  ...
  -t 
      Следующие типы являются допустимыми:
        ...
        iso:    Указание на ISO образ диска CD-ROM.  Геометрия определится 
                автоматически. Это может быть iso или cue/bin.
  ...
  -fs 
     Следующие форматы файловой системы допустимы:
        ...
        none: DOSBox не будет пытаться прочитать файловую систему на диске. Это 
              ...
              чтобы смонтировать образ в 70MB как slave диск, необходимо 
              набрать (без кавычек):
...
"Save". DOSBox сохранит раскладку в файл, заданную в настроечном файле DOSBox
...
"keyboardlayout" в секции [dos] настроечного файла DOSBox или внутренняя программа DOSBox
...
под Windows, раскладка выбирается на основании клавиатурной раскладки
...
Поддерживаемые внешние файлы
  ...
  .kl-file в директорию настроечного файла DOSBox и использованием первой части 
  имени файла как языкового кода.
...
или зависанию:
  ...
  - поменяйте некоторые записи в конфигурационном файле DOSBox, главным образом 
    ...
    а также установку параметра machine, уазывающего эмулируемую видеокарту и 
    функциональность:
      machine=vesa_nolfb
    или
      machine=vgaonly
...
Вы можете редактировать  сгенерированный фнастроечный файл для настройки DOSBox.
...
Файл настройки содержит текущие настройки. Вы можете изменить их 
и запустить DOSBox с параметром -conf, чтобы загрузить файл и использовать эти 
настройки.


DOSBox будет анализировать все насироечные файлы, указаные 
параметром -conf. Если не указан ни один настроечный файл DOSBox попытается
загрузить "dosbox.conf" в текущей директории. Если такого файла нет, DOSBox
загрузит пользовательский настроечный файл. Если этот файл отсутствует - он
будет создан. Этот файл располагается в ~/.dosbox (Linux) или 
"~/Library/Preferences" (MAC OS X). Пользователи Windows могут найти его
среди ярлыков в меню "Пуск".
...
Ищите адетс электронной почты на сайте: 
http://www.dosbox.com
(в разделе Crew).
